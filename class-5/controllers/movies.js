...
export class MovieController {
  ...
  update = async (req, res) => {
    ...
    if (!updateMovie) return res.status(404).json('Movie not found')

    return res.json(updateMovie)
  }

  delete = async (req, res) => {
    const result = await this.movieModel.delete({ id: req.params.id })
    if (!result) return res.status(404).json('Movie not found')
